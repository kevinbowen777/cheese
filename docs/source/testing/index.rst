...
::
    ...
    - coverage-3.14 -> Generate coverage reports.
    ...
    * pyright-3.14 -> Check types.
    * pyright-3.13 -> Check types.
    * pyright-3.12 -> Check types.
    * pyright-3.10 -> Check types
    * safety-3.14 -> Scan dependencies for insecure packages.
...
The ``lint``, ``safety``, and ``tests`` are enabled to be run with ``nox -s tests``. Generating documentation, (e.g. ``nox -s docs-3.14``) need to be run explicitly.
...
.. code-block:: console
   ...
   $ docker compose exec web nox -s coverage-3.13
