...
For clarity's sake, the following variables in the `.env` file need to be set manually:
 ...
 .. code-block:: console
    ...
    # export POSTGRES_DB=cheese.sqlite3
    DATABASE_URL=postgres://django_admin:<secret_key>@localhost:5432/cheese
    export POSTGRES_USER=django_admin
    export POSTGRES_PASSWORD=<postgres_password>
    export POSTGRES_DB=cheese
...
By default, the `.env` file is set up to run the `bookstore` project locally
